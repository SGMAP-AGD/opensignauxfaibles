...
source(find_rstudio_root_file('tools','interface','mark_known_sirets.R'))
...
```{r}
raw_data <- connect_to_database('Features', '1806', algo = 'algo2' ,min_effectif = 20)
```
...
prediction_month_before <- predict_at(actual_period %m-% months(1),
                         tw_complete_long,
                         function(x) caret_prob(rf,x))

joined <-
  compare_prediction_probabilities(
    sample_start = prediction_month_before,
    sample_end = prediction,
    only_siret = TRUE) %>%
  left_join(raw_data, by = c('siret', 'periode')) 

# Export
joined %>% 
  prepare_for_export(additional_names = c('diff','prob_old', 'connu')) %>% 
  export(batch = '1806')
```

...
distrib = c(grep('APE1',names(raw_data), value = TRUE),
            grep('APE1',names(raw_data), value = TRUE), 
...
pred_data <- all_data %>%
  left_join(prediction %>% rename(prob = default0) %>% select(predict, prob,siret, periode), by=c('siret', 'periode')) %>% 
  group_by(siret) %>%
  arrange(siret,periode) %>% 

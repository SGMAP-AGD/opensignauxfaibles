objective_default_or_failure <- function(data,n_months, threshold, lookback){
  ...
  data <- data %>%
    ...
    mutate(default_aux = check_n_successive_defaults(dette_cumulee_aux,n_months,threshold),
           failure_aux = etat_proc_collective %in% proc,
           default_any = any(default_aux | failure_aux),
           failure = with_lookback(failure_aux, lookback),
           default = with_lookback(default_aux | failure_aux,lookback)) %>%
    ungroup() %>%
    select(-default_aux, -failure_aux, dette_cumulee_aux)
